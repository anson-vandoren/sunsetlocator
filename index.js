...
function heliocentricLongitude(d) {
  ...
  const longitudeInRadians =
    (longitude[0] +
      ...
      longitude[5] * jme ** 5) /
    1e8;
  return limit_degrees(rad2deg(longitudeInRadians));
}

function earthRadiusVector(d) {
  //https://www.nrel.gov/docs/fy08osti/34302.pdf Equations 9-12
  const jme = julianEphemerisMillenium(d);
  const earthPeriodicSum = (acc, row) => {
    const curVal = row[1] * Math.cos(row[2] + row[3] * jme);
  ...
  const earthRadiusRows = [0, 1, 2, 3, 4];
  const earthRadius = earthRadiusRows.map(idx =>
    earthRadiusConstants[idx].reduce(earthPeriodicSum, 0)
  );
  const earthRadiusInRadians =
    (earthRadius[0] +
      earthRadius[1] * jme +
      earthRadius[2] * jme ** 2 +
      earthRadius[3] * jme ** 3 +
...
function heliocentricLatitude(d) {
  //https://www.nrel.gov/docs/fy08osti/34302.pdf Equations 9-12
  const jme = julianEphemerisMillenium(d);
  const earthPeriodicSum = (acc, row) => {
    const curVal = row[1] * Math.cos(row[2] + row[3] * jme);
  ...
  const latitudeRows = [0, 1];
  const latitude = latitudeRows.map(idx =>
    latitudeConstants[idx].reduce(earthPeriodicSum, 0)
  );
  const latitudeInRadians = (latitude[0] + latitude[1] * jme) / 1e8;
  console.log("latrad", latitudeInRadians);
